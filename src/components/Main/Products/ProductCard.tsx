...
const ProductCard = ({
  ...
  return (
    <>
      <article
        className={`${className} card group relative flex flex-col justify-between overflow-hidden pb-4 hover:!border-secColor`}
      >
        {" "}
          ...
          <div className="p-2">
            <hgroup className="flex flex-col justify-between">
              <h4 className="text-xs font-extrabold text-pryColor lg:text-sm">
                {productData?.name} {parseFloat(productData?.weight as string)}
                kg{" "}
              </h4>
        ...
        <div className="container flex flex-wrap justify-between gap-3">
          ...
          <Button
            onClick={() => handleShow(`todays-deal-${productData?.id}`)}
            type="button"
            className="w-full"
            loading={isLoading}
            disabled={productData?.deal_end_date ? true : false}
          >
